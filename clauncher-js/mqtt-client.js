...
export default class MqttClient {
    ...
    constructor(st) {
        ...
        this.settings = {
            ...
            clientid: st.clientid !== undefined ? st.clientid : 'this.mqttc-client-' + Math.round(Math.random() * 10000),
            ...
            willMessage: st.willMessage !== undefined ? new Paho.Message(st.willMessage) : undefined,
        ...
        _this = this;
        setTimeout(this.testmsg, 2000);
    }

    async connect() {
        // init Paho client connection
        this.mqttc = new Paho.Client(this.settings.host, Number(this.settings.port), this.settings.clientid);
                ...
                onSuccess: () => { 
                    if (_this.settings.subscribeTopics != undefined) {
                        ...
                        if (_this.settings.subscribeTopics.length > 0) {
                            if (_this.settings.dbg == true) console.log('Subscribing to: ' + _this.settings.subscribeTopics + '\n');
                            _this.mqttc.subscribe(_this.settings.subscribeTopics);
                        }
                    }
                    if (_this.settings.onConnectCallback != undefined) _this.settings.onConnectCallback(_this.settings.onConnectCallbackContext);
                    resolve();
                },
    ...
    testmsg() {
        let fn = "counter.wasm";
        //let fn = "counter.wasm";
        let msg = new Paho.Message('{"object_id": "fcb2780b-abdd-43b6-bc13-895baa2075a2", "action": "create", "type": "arts_req", "data": {"type": "module", "details": {"uuid": "44c72c87-c4ec-4759-b587-30ddc8590f6b", "name": "test", "parent": {"uuid": "' +_this.settings.clientid+ '"}, "filename": "' + fn + '", "fileid": "na", "filetype": "WA", "args": ""}}}');
    ...
    onConnectionLost(responseObject) {
        ...
        if (responseObject.errorCode !== 0) {
            if (this.settings.dbg == true) console.log('Mqtt ERROR: ' + responseObject.errorMessage + '\n');
        }
    }
