 ...
 */
cwlib_set_timer(int delay_ms, cwlib_event_handler_t timer_callback, void *ctx);

/**
 * Polls timers and files. Performs callbacks appropriately
 * Cwlib user should call cwlib_poll() in a loop
 *
 * @param sleep_s amount of seconds to sleep if we have no events to wait
 * 
 */
void cwlib_poll(int sleep_s);
